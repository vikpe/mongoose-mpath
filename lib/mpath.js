...
function mpathPlugin(schema, options) {
  ...
  schema.pre('save', function preSave(next) {
    ...
    if (this.parent) {
      this.collection
          .findOne({_id: new ObjectId(this.parent)})
          .then((parentDoc) => {
  ...
  schema.pre('remove', function preRemove(next) {
    ...
    if ('DELETE' === onDelete) {
      ...
      // TODO probably not working……also not the same as pre save hook which seems to be working
      streamWorker(
          childCursor,
          onStreamData,
          streamWorkerOptions,
  ...
  const getLevelByPath = (path) =>
    path ? path.split(pathSeparator).length : 0;
  ...
  schema.statics.getChildrenTree = function getChildrenTree(args) {
    ...
    return this.find(filters, fields, options)
        ...
        .then((results) => {
          const createChildren = (result, node, level) => {
            if (level === minLevel) {
              node.children = [];
          ...
          for (let key in results) {
            if (results.hasOwnProperty(key)) {
              const level = getLevelByPath(results[key].path);
