...
function mpathPlugin(schema, options) {
  ...
  const getLevelByPath = path => (path ? path.split(pathSeparator).length : 0);
  ...
  schema.statics.getChildrenTree = function getChildrenTree(rootDoc, args) {
  ...
  schema.methods.getChildrenTree = function(args) {
    return this.constructor.getChildrenTree(this, args);
  };
}
